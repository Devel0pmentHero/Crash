...
abstract class Test {
    ...
    final public function __invoke(): array {
        ...
        $Assert    = static function(string $File, int $Line, string $Assertion, ?string $Description = null) use ($Start, $Allocated): array {
            return [
        ...
        $Error     = static function($Code, string $Message, string $File, int $Line, ?array $Context = []) use ($Start, $Allocated): array {
            return [
        ...
        $Summary   = static function(array $Cases): array {
            ...
            $Failed = \count(\array_filter(
                $Cases,
                static function(array $Case): bool {
                    return $Case[self::Result] === self::Failed;
                }
            ));
            if((bool)$Failed) {
                $Status = self::Failed;
            }
            $Crashed = \count(\array_filter(
                $Cases,
                static function(array $Case): bool {
                    return $Case[self::Result] === self::Crashed;
                }
            ));
            if((bool)$Crashed) {
                $Status = self::Crashed;
            ...
            return [
                ...
                self::Duration  => \array_reduce(
                    $Cases,
                    static function(float $Sum, array $Case): float {
                        return $Sum += $Case[self::Duration] ?? 0;
                    },
                    0
                ),
                self::Allocated => \array_reduce(
                    $Cases,
                    static function(float $Sum, array $Case): float {
                        return $Sum += $Case[self::Allocated] ?? 0;
                    },
                    0
                ),
        ...
        foreach($Class->getMethods() as $Method) {
            ...
            if(\PHP_VERSION_ID >= 80000) {
                //Skip Test case.
                foreach($Method->getAttributes(Test\Method\Skip::class) as $Attribute) {
                    $Cases[$Method->getName()] = [self::Result => self::Skipped];
                    continue 2;
                }

                //Repeat Test case.
                foreach($Method->getAttributes(Test\Method\Repeat::class) as $Attribute) {
                    $Repetitions = [];
                    foreach(Test\Method\Repeat::FromReflector($Attribute) as $Repetition) {
                        //Overwrite error handlers.
                        \assert_options(
                            \ASSERT_CALLBACK,
                            static function(...$Values) use (&$Repetitions, $Repetition, $Assert) {
                                $Repetitions[$Repetition] = $Assert(...$Values);
                            }
                        );
                        \set_error_handler(
                            static function(...$Values) use (&$Repetitions, $Repetition, $Error) {
                                $Repetitions[$Repetition] = $Error(...$Values);
                            }
                        );
                ...
                //Crash Test case.
                foreach($Method->getAttributes(Test\Method\Crash::class) as $Attribute) {
                    $Repetitions = [];
                    foreach(Test\Method\Crash::FromReflector($Attribute) as $Repetition => $Values) {
                        //Overwrite error handlers.
                        \assert_options(
                            \ASSERT_CALLBACK,
                            static function(...$Values) use (&$Repetitions, $Repetition, $Assert) {
                ...
                //Apply values.
                foreach($Method->getAttributes(Test\Method\Values::class) as $Attribute) {
                    $Cases[$Method->getName()] = $Run($Method, ...Test\Method\Values::FromReflector($Attribute)->Values);
                    continue 2;
                }
            }
            ...
            \assert_options(
                \ASSERT_CALLBACK,
                static function(...$Values) use (&$Cases, $Method, $Assert) {
                    $Cases[$Method->getName()] = $Assert(...$Values);
                }
            );
            \set_error_handler(
                static function(...$Values) use (&$Cases, $Method, $Error) {
                    $Cases[$Method->getName()] = $Error(...$Values);
                }
            );
